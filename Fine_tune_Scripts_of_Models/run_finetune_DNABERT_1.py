...
def predict(args, model, tokenizer, prefix=""):
    # Loop to handle MNLI double evaluation (matched, mis-matched)
    checkpoint_path = "/vol/space/DNABERT/Data/HNRNAPL/output/pytorch_model.bin"
    
    # Ensure the model is loaded from the specified checkpoint
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        # Load the dataset; check if a test file is specified
        hardcoded_test_file_path = '/vol/space/DNABERT/Data/HNRNAPL/DNB1/test.tsv'
        pred_dataset = load_and_cache_examples(args, pred_task, tokenizer, evaluate=False, test_file=hardcoded_test_file_path)
        ...
        output_pred_file = os.path.join(pred_output_dir, "pred_results_DNB1")
        logger.info("***** Pred results {} *****".format(prefix))
        for key in sorted(result.keys()):
           logger.info("  %s = %s", key, str(result[key]))
        np.save(output_pred_file, probs)
       # Save as CSV
        with open(output_pred_file, "w", newline="") as csvfile:
         writer = csv.writer(csvfile)
         writer.writerow(["index", "predictions"])  # Add a header row
         for i, preds in enumerate(preds):
           writer.writerow([i + 1, preds])  # Write index and probability
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False , test_file=None):
    ...
    if args.do_predict:
     ...
     # Determine mode and set appropriate cache file name
    mode = "dev" if evaluate else "train"
    if args.do_predict and test_file:
        mode = "test"
        dataset_path = test_file  # Use test_file path for loading test dataset
    else:
        dataset_path = args.data_dir  # Default to data directory

    # Build the cache file name based on mode
    cached_features_file = os.path.join(
        args.data_dir,
        "cached_{}_{}_{}_{}".format(
            mode,
...
def main():
    ...
    default=None,
    type=str,
    help="Path to the test file to be used for predictions."
)
    # Other parameters
    parser.add_argument(
        "--visualize_data_dir",

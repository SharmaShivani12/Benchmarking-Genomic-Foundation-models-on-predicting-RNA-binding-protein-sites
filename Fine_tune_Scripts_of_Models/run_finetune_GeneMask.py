...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # changing for the multi-label setting

            if args.task_name == "dnachromatin":
                ...
                #print('Outputs: ', type(outputs), outputs, outputs[0].size, 'Batch: ', batch[3])
                loss = loss_fn(outputs[0], batch[3])
            else:
...
def evaluate(args, model, tokenizer, prefix="", evaluate=True):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                if args.task_name == 'dnachromatin':
                    ...
                    outputs = model(**inputs)
                    logits= outputs[0].sigmoid()
                    tmp_eval_loss = loss_fn(outputs[0], batch[3])
                else:
                    inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3]}
                    if args.model_type != "distilbert":
                        inputs["token_type_ids"] = (
            ...
            # over multiple batches the results are appended to a single list
            if preds is None:
        ...
        if args.output_mode == "classification":
            if args.task_name[:3] == "dna" and args.task_name != "dnasplice" and args.task_name != "dnachromatin":
...
def predict(args, model, tokenizer, prefix=""):
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        ...
        output_pred_file = os.path.join(pred_output_dir, "pred_results_GEN.npy")
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if args.n_process == 1: # or args.task_name == "dnachromatin": #added
            features = convert_examples_to_features(
            examples,
            tokenizer,
            task= args.task_name, # added
            ...
            for i in range(n_proc):
                ...
                results.append(p.apply_async(convert_examples_to_features, args=(examples[indexes[i]:indexes[i+1]], tokenizer, max_length, args.task_name, label_list, output_mode, pad_on_left, pad_token, pad_token_segment_id, True,  )))
    ...
    if output_mode == "classification":
        if args.task_name == 'dnachromatin':
            all_labels = torch.tensor([f.label for f in features], dtype=torch.float) # here, f.label is a numpy array
        else:

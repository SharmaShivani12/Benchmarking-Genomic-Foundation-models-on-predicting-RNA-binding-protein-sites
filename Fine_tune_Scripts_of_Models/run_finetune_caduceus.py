...
dataset_path = "/vol/space/caduceus/Data/QKI"

# Load your dataset and tokenize it
dataset = load_dataset("csv", data_files={"train": f"{dataset_path}/train.csv", 
                                           "validation": f"{dataset_path}/val.csv",
...
def compute_metrics(pred):
    ...
    loss = pred.loss if hasattr(pred, "loss") else None
...
training_args = TrainingArguments(
    output_dir='/vol/space/caduceus/Data/QKI/output',  # Outputs and models will be saved here, dynamically set with date
    evaluation_strategy="steps",  # Evaluate every 'eval_steps'
    eval_steps=200,  # Number of steps to run evaluation
    save_steps=200,  # Number of steps to save the model
    save_total_limit=10,  # Max number of checkpoints to keep
    learning_rate=3e-5,  # Learning rate
    #lr_scheduler_type="cosine",  # Type of learning rate scheduler
    warmup_ratio=0.1,  # Proportion of training to perform linear learning rate warmup
    ...
    optim="adamw_torch",
    load_best_model_at_end=True  # Optimizer type: using PyTorch's native AdamW
) 
...
results_df.to_csv("test_predictions_caduceus_QKI.csv", index=False)
...
print("Saved test predictions to 'test_predictions_caduceus.csv'")
